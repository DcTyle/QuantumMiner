# ASCII-ONLY
# BTC Stratum job adapter (Stratum V1)
...
class BTCAdapter(CoinAdapter):
    ...
    def on_response(self, msg: Dict[str, Any]) -> None:
        # Capture subscribe result for extranonce details if present
        if not isinstance(msg, dict):
            return
        if msg.get("result") and isinstance(msg.get("result"), list):
            try:
                res = msg.get("result", [])
                # mining.subscribe result often: [ [sub_details], extranonce1, extranonce2_size ]
    ...
    def convert_job(self, job_raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Height may be present as extra param or dict
        height = None
        try:
            if len(params) > 9:
                cand = params[9]
        ...
        out.update({
            "job_id": str(job_id or out.get("job_id", "BTC_job")),
            "nbits": str(nbits),
            "ntime": str(ntime),
            "extranonce1": en1,
